...
final class FakerFromPathSpec: QuickSpec {
  override func spec() {
    describe("Faker") {
      var faker: Faker!
      let testingLocale = "en-GB"

      beforeEach {
        let testBundle = Bundle(for: type(of: self))
        let path = testBundle.path(forResource: testingLocale, ofType: "json")
        faker = Faker(locale: testingLocale, path: path!)
      }

      describe("#init") {
        it("sets default values") {
          expect(faker.locale).to(equal(testingLocale))
          expect(faker.parser.locale).to(equal(testingLocale))
